...
// Statü anahtarları (frontend ile birebir aynı olmalı!)
const STATUS_KEYS = [
  "aranmadi",
  "tekrar_aranacak",
  "yakin_takip",
...
router.put("/:id/details", verifyUser, async (req, res) => {
  try {
    ...
    if (!details) return res.status(400).json({ message: "Eksik veri." });

    // Otomatik toplam hesapla ve ekle
...
// İstatistik endpoint (PlanetScale için backend toplama!)
